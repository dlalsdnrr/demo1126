...
class VoiceAssistant:
    def __init__(self) -> None:
        ...
        self.KEYWORDS = {
            ...
            "안타": ["안타", "앙타", "안 타", "암타", "안탈", "안탑", "아타", "안타는", "안타가", "아안타", "안타개수",
                   "안나", "안타로", "안다", "안달", "았다"]
            # TODO: "타점", "2루타", "득점" 등 키워드 추가 필요
        }
        
        # 💡 [수정됨] 2025 PO 16명 선수 별칭 (유사 발음) 대폭 보강
        self.PLAYER_ALIASES = {
        ...
        # 💡 [수정됨] 2025 PO 16명 선수 데이터로 업데이트 (타율, 홈런, 안타)
        self.PLAYERS_DATA = {
            # 1. 김영웅 (삼성): 0.625 ... 16 10 3 0 3 12 ... (H: 10, HR: 3으로 추정)
            "김영웅": { "타율": 0.625, "홈런": 3, "안타": 10 },
            # 2. 문현빈 (한화): 0.444 ... 18 8 3 0 2 10 ... (H: 8, HR: 2로 추정 - H는 AVG*AB로 보정)
            "문현빈": { "타율": 0.444, "홈런": 2, "안타": 8 },
            # 3. 노시환 (한화): 0.429 ... 21 9 2 1 2 5 ... (H: 9, HR: 2로 추정 - H는 AVG*AB로 보정)
            "노시환": { "타율": 0.429, "홈런": 2, "안타": 9 },
            # 4. 리베라토 (한화): 0.389 ... 18 7 1 0 1 2 ... (H: 7, HR: 1로 추정)
            "리베라토": { "타율": 0.389, "홈런": 1, "안타": 7 },
            # 5. 김태훈 (삼성): 0.353 ... 17 6 0 0 2 2 ... (H: 6, HR: 2로 추정)
            "김태훈": { "타율": 0.353, "홈런": 2, "안타": 6 },
            # 6. 최재훈 (한화): 0.353 ... 17 6 0 0 0 0 ... (H: 6, HR: 0로 추정)
            "최재훈": { "타율": 0.353, "홈런": 0, "안타": 6 },
            # 7. 채은성 (한화): 0.350 ... 20 7 1 0 0 8 ... (H: 7, HR: 0로 추정)
            "채은성": { "타율": 0.350, "홈런": 0, "안타": 7 },
            # 8. 하주석 (한화): 0.350 ... 20 7 2 0 0 1 ... (H: 7, HR: 0로 추정)
            "하주석": { "타율": 0.350, "홈런": 0, "안타": 7 },
            # 9. 구자욱 (삼성): 0.313 ... 16 5 1 0 0 3 ... (H: 5, HR: 0로 추정)
            "구자욱": { "타율": 0.313, "홈런": 0, "안타": 5 },
            # 10. 이재현 (삼성): 0.294 ... 17 5 2 0 1 4 ... (H: 5, HR: 1로 추정)
            "이재현": { "타율": 0.294, "홈런": 1, "안타": 5 },
            # 11. 디아즈 (삼성): 0.278 ... 18 5 2 0 0 2 ... (H: 5, HR: 0로 추정)
            "디아즈": { "타율": 0.278, "홈런": 0, "안타": 5 },
            # 12. 손아섭 (한화): 0.263 ... 19 5 2 0 0 2 ... (H: 5, HR: 0로 추정)
            "손아섭": { "타율": 0.263, "홈런": 0, "안타": 5 },
            # 13. 김성윤 (삼성): 0.261 ... 23 6 0 0 0 0 ... (H: 6, HR: 0로 추정)
            "김성윤": { "타율": 0.261, "홈런": 0, "안타": 6 },
            # 14. 김지찬 (삼성): 0.190 ... 21 4 0 1 0 0 ... (H: 4, HR: 0로 추정)
            "김지찬": { "타율": 0.190, "홈런": 0, "안타": 4 },
            # 15. 강민호 (삼성): 0.188 ... 16 3 0 0 1 3 ... (H: 3, HR: 1로 추정)
            "강민호": { "타율": 0.188, "홈런": 1, "안타": 3 },
            # 16. 심우준 (한화): 0.077 ... 13 1 1 0 0 0 ... (H: 1, HR: 0로 추정)
    ...
    def _transcribe_etri(self, audio_data: bytes) -> Optional[str]:
        ...
        http_headers = {
            # 💡 config.ETRI_API_KEY 사용
            "Authorization": config.ETRI_API_KEY, 
            "Content-Type": "application/json; charset=UTF-8",
        }
        
        try:
            # 💡 config.ETRI_API_URL 사용
    ...
    def _get_reply(self, text: str, player_name: Optional[str], keyword: Optional[str]) -> str:
        ...
        # 💡 [수정됨] 키워드 응답 포맷 변경 (타율 외에는 .3f 포맷 제거)
        if not text:
        ...
        if value is None:
            # 💡 [수정됨] 타율/홈런/안타 외의 키워드를 처리하기 위한 임시 응답
            # (현재는 KEYWORDS에 없어서 이 부분이 실행될 일은 없습니다)
        ...
        if keyword == "타율":
            ...
            # 💡 [수정됨] 향후 다른 키워드(타점, 득점 등) 추가 시 기본 응답
