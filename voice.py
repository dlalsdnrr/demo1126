...
# ------------------------------------------------------------------
        ...
        self.KEYWORDS = {
            ...
            "홈런": ["홈런", "홈롬", "HR", "담장"],
            "안타": ["안타", "안 타", "H"],
            "타점": ["타점", "RBI", "점수 낸 거", "불러들인 거"],
            "득점": ["득점", "R", "들어온 거", "홈인"],
            "2루타": ["2루타", "2B", "이루타"],
            "3루타": ["3루타", "3B", "삼루타"],
            
            # --- 투수 기록 ---
            "평균자책점": ["평균자책점", "평균자책", "자책점", "방어율", "ERA"],
            "승리": ["승리", "승", "승수", "W", "다승"],
            "패배": ["패배", "패", "패수", "L"],
            "세이브": ["세이브", "SV", "S", "뒷문"],
            "홀드": ["홀드", "HLD"],
        ...
        # ------------------------------------------------------------------
        # 3. 별명 자동 등록 로직 (데이터에 있는데 별명이 없으면 자동 생성)
        # ------------------------------------------------------------------
        ...
        print(f"✓ 검색어(별명) 준비 완료: {len(self.PLAYER_ALIASES)}개")
    ...
    def generate_gemini_response(self, user_query: str) -> str:
        ...
        # 지침:
        1. 반드시 한국어로, 친절하고 정중하게 비서처럼 답변하세요.
